...
# Error Viz Class for Classification


class ErrorVizClassification:
    ...
    def _transform(self, df, col, pred):
        ...
        agg_err = np.nanmedian(err_arr)
        err_arr = agg_err - err_arr
        # errs = df["err_fin"].values
        agg_err = pd.DataFrame(
            {
                col: df[col].mode(),
                pred: np.nanmedian(df[pred]),
                "errPos": np.nanmedian(err_arr[err_arr >= 0]),
                "errNeg": np.nanmedian(err_arr[err_arr <= 0]),
            },
            index=[0],
        )
    ...
    def revalue_numeric(self, data, col):
    ...
    def prob_acc(self,true_class=0, pred_prob=0.2):
        """
    ...
    def create_plot_categorical(self, feature, groupbyvar=None, groupbyvalue=None):
        ...
        if groupbyvar is not None:
            ...
            pred = self.modelobj.predict_proba(self.model_df[self.model_df[gb_title] == 1])
            ...
            pred = self.modelobj.predict_proba(self.model_df)
        ...
        plot_df = self.revalue_numeric(plot_df, col = "xaxis")
    ...
    def create_plot_continuous(
        ...
        if groupbyvar is not None:
            ...
            # Make predictions for groupby value constraint
            preds = self.modelobj.predict_proba(self.model_df[self.model_df[gb_title] == 1])
            ...
            plot_title = feature
            preds = self.modelobj.predict_proba(self.model_df)
            # select the prediction probabilities for the class labeled 1
            preds = preds[:, 1].tolist()
        ...
        actual = plot_df.loc[:, "true"].apply(lambda x: class_lookup[x]).values.tolist()

        # calculate the difference between actual and predicted probabilities
        diff = [self.prob_acc(true_class=actual[idx], pred_prob=pred) for idx, pred in enumerate(preds)]

        # assign errors
        plot_df["err"] = diff

        # Check data length and return data
        plot_df = self.revalue_numeric(plot_df, col = "xaxis")

        # assign error values to variable
        # err_arr = plot_df["err"].values

        # get user defined aggregate (central values) of the errors
        # agg_errs = np.nanmedian(err_arr)

        # subtract the aggregate value for the group from the errors
        # err_arr = agg_errs - err_arr

        # assign final error values
        # plot_df['err_fin'] = err_arr
    ...
    def create_combined_cat_plot(self, feature, groupbyvar=None):
        ...
        if groupbyvar is None:
            ...
            for i in range(len(groupbyvalues)):
                ...
                pred = self.modelobj.predict_proba(
